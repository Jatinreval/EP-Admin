/**
 * Reminder re Draft handler
 * This module handles email generation for reminders about document drafts
 */
...
function initialize() {
  console.log("Initializing Reminder re Draft Documents handler");
  
  // Remove any existing event listeners first to prevent duplicates
  const generateButton = document.getElementById("reminder-draft-generate");
  const replyButton = document.getElementById("reminder-draft-reply-button");
  ...
  console.log("Event listeners initialized for Reminder re Draft Documents handler");
}

/**
 * Generates a draft document reminder email
 */
async function generateDraftReminder() {
  const statusElement = document.querySelector('.reminder-status');
  const responseContainer = document.getElementById('reminder-draft-response-container');
  const replyButton = document.getElementById('reminder-draft-reply-button');
  const copyButton = document.getElementById('reminder-draft-copy-button');
  
  // Show the loader
  showLoader();
  
  updateStatus("Generating draft document reminder email...", '.reminder-status');
  ...
  try {
    ...
    const subject = `Reminder: Document Draft - ${clientInfo.subject || clientInfo.name || 'Review Requested'}`;
    ...
    updateStatus("Draft document reminder email created successfully!", '.reminder-status');
  } catch (error) {
    updateStatus(`Error: ${error.message}`, '.reminder-status');
    console.error("Error generating draft document reminder email:", error);
  } finally {
  ...
  try {
    ...
    const payload = {
      messages: [
        {
          role: "system",
          content: "You are an AI assistant specialized in drafting professional legal communications. Your task is to create a well-structured reminder email to a client regarding a document draft that requires their attention or approval. Format your response using markdown for bold text (**bold**) and proper structure. IMPORTANT: Output ONLY the email content itself - do NOT include any analysis, explanations, or headers/notes before the actual email."
        },
        {
          role: "user",
          content: `Draft a professional reminder email to a client about their document draft. Include these details:

Client Name: ${clientInfo.name || 'Valued Client'}
Client Email: ${clientInfo.email || '[Client Email]'}

Email Content Guidelines:
1. Begin with a subject line: "**Subject: Reminder: Document Draft - ${clientInfo.subject || 'Review Requested'}**"
2. Next, start with "**Private and Confidential**"
3. Begin with a formal greeting to the client
4. Reference the draft document(s) previously sent to them
5. Remind them that you are still waiting for their review, comments, or approval
6. Highlight the importance of their feedback to proceed with their matter
7. Provide a suggested deadline for their response
8. Offer to discuss any questions or concerns they may have about the draft
9. Request they contact your office at their earliest convenience
10. End with a formal closing and signature

Keep the tone professional and helpful, emphasizing the importance of their input while maintaining a courteous approach. Format any specific document names or dates clearly.`
        }
      ],
      temperature: 0.7,
      max_tokens: 500,
