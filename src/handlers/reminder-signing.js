/**
 * Reminder re Signing handler
 * This module handles email generation for reminders about signing documents
 */
...
function initialize() {
  console.log("Initializing Reminder re Signing Documents handler");
  
  // Remove any existing event listeners first to prevent duplicates
  const generateButton = document.getElementById("reminder-signing-generate");
  const replyButton = document.getElementById("reminder-signing-reply-button");
  ...
  console.log("Event listeners initialized for Reminder re Signing Documents handler");
}

/**
 * Generates a signing document reminder email
 */
async function generateSigningReminder() {
  const statusElement = document.querySelector('.reminder-status');
  const responseContainer = document.getElementById('reminder-signing-response-container');
  const replyButton = document.getElementById('reminder-signing-reply-button');
  const copyButton = document.getElementById('reminder-signing-copy-button');
  
  // Show the loader
  showLoader();
  
  updateStatus("Generating signing document reminder email...", '.reminder-status');
  ...
  try {
    ...
    // Format a nice subject for the email
    const subject = `Reminder: Signing Appointment - ${clientInfo.subject || clientInfo.name || 'Your Documents'}`;
    ...
    // Format the email as HTML for creating a new message
    ...
    updateStatus("Signing document reminder email created successfully!", '.reminder-status');
  } catch (error) {
    updateStatus(`Error: ${error.message}`, '.reminder-status');
    console.error("Error generating signing document reminder email:", error);
  } finally {
...
async function generateEmailFromPrompt(clientInfo) {
  try {
    ...
    // Format meeting times as bullet points
    const meetingTimes = [
    ...
    const payload = {
      messages: [
        {
          role: "system",
          content: "You are an AI assistant specialized in drafting professional legal communications. Your task is to create a well-structured reminder email to a client regarding their appointment to sign documents. Format your response using markdown for bold text (**bold**) and proper structure. IMPORTANT: Output ONLY the email content itself - do NOT include any analysis, explanations, or headers/notes before the actual email."
        },
        {
          role: "user",
          content: `Draft a professional reminder email to a client about their document signing appointment. Include these details:
...
1. Begin with a subject line: "**Subject: Reminder: Document Signing Appointment - ${clientInfo.subject || 'Your Documents'}**"
2. Next, start with "**Private and Confidential**"
3. Begin with a formal greeting to the client
4. Reference the documents that are ready for signing
5. Remind them that we are waiting to confirm a time for their signing appointment
6. Propose the available meeting times listed above as bullet points
7. Emphasize the importance of finalizing their documents
8. Provide a suggested deadline for their response
9. Offer to discuss any questions or concerns they may have
10. End with a formal closing and signature

Keep the tone professional and helpful, emphasizing the importance of completing the signing process while maintaining a courteous approach.`
        }
      ],
      temperature: 0.7,
      max_tokens: 500,
